...
class ImageEditorPanel extends JPanel {
    ...
    private static final int SECTOR_RIGHT = 1;
    private static final int SECTOR_BOTTOM = 2;
    private static final int SECTOR_LEFT = 3;
    ...
    File chooseSaveFile(JFrame parent) {
        ...
        System.out.println("selected file: " + dialog.getFile());
        if (dialog.getDirectory() != null && dialog.getFile() != null) {
            File saveFile = new File(dialog.getDirectory(), dialog.getFile());

            if (!saveFile.getAbsolutePath().endsWith(EXTENSION_9PATCH)) {
    ...
    private class ImageViewer extends JComponent {
        ...
        private boolean isPointOverLine(int x, int y, float slope, float intercept) {
            float yForX = x * slope + intercept;
        ...
        private boolean isSoutheast(int x, int y) {
            int h = image.getHeight();
            int w = image.getWidth();
            float slope = (float)-h / w;
            float intercept = h;

            return isPointOverLine(x, y, slope, intercept);
        }

        private boolean isSouthwest(int x, int y) {
            int h = image.getHeight();
            int w = image.getWidth();
            float slope = (float)h / w;
            float intercept = 0;
        ...
        private int getSectorOfPoint(int x, int y) {
            boolean isSoutheast = isSoutheast(x, y);
            boolean isSouthwest = isSouthwest(x, y);

            if (isSoutheast && isSouthwest) {
                return SECTOR_BOTTOM;
            } else if (isSoutheast && !isSouthwest) {
                return SECTOR_RIGHT;
            } else if (!isSoutheast && isSouthwest) {
                return SECTOR_LEFT;
            } else if (!isSoutheast && !isSouthwest) {
        ...
        private int getClampedX(int x, int y) {
            switch (getSectorOfPoint(x, y)) {
                case SECTOR_TOP:
                case SECTOR_BOTTOM:
                default:
                    return x;
        ...
        private int getClampedY(int x, int y) {
            switch (getSectorOfPoint(x, y)) {
                case SECTOR_TOP:
                    return 0;
                case SECTOR_BOTTOM:
        ...
        private boolean paintCoord(int x, int y, int color) {
            x = getClampedX(x, y);
            y = getClampedY(x, y);
        ...
        private boolean fillInterveningPaintCoords(int nextPaintX, int nextPaintY, int color) {
            ...
            for (int x = x0; x < x1; x++) {
                if (steep) {
                    wasPainted |= paintCoord(y, x, color);
                } else {
                    wasPainted |= paintCoord(x, y, color);
                }
                error += dError;
                if (error >= 0.5) {
                    y += yStep;
        ...
        private void paint(int x, int y, int button, boolean fillIntervening) {
            int color;
            switch (button) {
            ...
            if (fillIntervening) {
                fillInterveningPaintCoords(x, y, color);
            }
